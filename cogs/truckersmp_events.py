...
class TruckersMPEvents(commands.Cog):
    ...
    async def event(
        ...
        vtc_logo = None
        if data.get("vtc"):
            vtc_name = data["vtc"].get("name")
            vtc_logo = data["vtc"].get("logo")
        ...
        embed = discord.Embed(
            title=title,
            url=url,
            description=f"**Hosted by:** {vtc_name}" if vtc_name else description,
        ...
        route_embed = None
        if route_image:
            route_embed = discord.Embed(title="🗺 Event Route", color=discord.Color.green())
    ...
    async def reminder_loop(self):
        ...
        for row in rows:
            (
                event_id, guild_id, role_id, event_date,
                title, server, start_time,
                banner, slot_number, slot_image, route_image
            ) = row
                ...
                if now_ist.date() == event_day and now_ist.hour == 7:
                    ...
                    for member in role.members:
                        try:
                            ...
                            embed.add_field(name="Time (IST)", value=start_time)
                            embed.add_field(name="Slot Number", value=str(slot_number))

                            if banner:
                                embed.set_image(url=banner)

                            if slot_image:
                                embed.set_thumbnail(url=slot_image)

                            if route_image:
                                embed.add_field(
                                    name="Route",
                                    value=f"[View Route]({route_image})",
                                    inline=False
                                )
